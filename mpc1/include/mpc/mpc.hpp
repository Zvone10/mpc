...
public:
    ...
    Symbolic A_d_uk1_sym_;		//A_d^i	,	i = 1, ... , Hp
    Symbolic A_d_uk2_sym_;		//(sum(A_d^i))*B_d  ,  i = 0, ... , Hp -1
...
MPC::MPC()
{
	...
	/*** Total matrices ***/
    A_d_uk1_sym_ = Symbolic ("A_d_uk1_sym_", 4, 4);
    A_d_uk2_sym_ = Symbolic ("A_d_uk2_sym_", 4, 4);
...
void MPC::getPredictModelState(){
	...
	A_d_uk2_ = A_d_uk2_ * B_d_;

	for (n_ = 0; n_ < 4; n_++ ){
		for (m_ = 0; m_ < 4; m_++){
			A_d_uk1_sym_(n_,m_) = A_d_uk1_(n_,m_);
			A_d_uk2_sym_(n_,m_) = A_d_uk2_(n_,m_);
		}
	}
    ...
    xp_ = A_d_uk1_sym_ * x_sym_ + A_d_uk2_sym_ * ulaz_.column(kk_);
